...
from models import  STDTransformer, AutoCoTransformer, Linear_block, STDRNN_singlelayer, STDRNN_multilayer,RNN_block
...
class Exp_Main_New(Exp_Basic):
    def __init__(self, args):
        super(Exp_Main_New, self).__init__(args)
    ...
    def vali(self, vali_data, vali_loader, criterion):
        if self.args.model == 'SingleRNN' or self.args.model == 'RNNBlock':
            print('RNN model evaluation')
            self.model.totrain = False
        total_loss = []
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                pred = outputs.detach().cpu()
                true = batch_y.detach().cpu()
                true_detec = torch.where(true.abs() < 1000, 0, true)
                abnormal_value = true[torch.nonzero(true_detec)]
