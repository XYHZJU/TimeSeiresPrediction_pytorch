...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.new_encoder = nn.ModuleList(self.new_encoder)
        self.new_decoder = nn.ModuleList(self.new_decoder)
        self.outputlinear = nn.Linear(2 * configs.d_model, configs.c_out)
        self.finaloutputlinear = nn.Linear(configs.enc_in*configs.enc_in, configs.c_out)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        for i in range(self.input_dim):
            ...
            #print(x_enc.shape)
            x_enc_i = x_enc[:,:,i].unsqueeze(2).to(x_enc.device)
            x_dec_i = x_enc[:,:,i].unsqueeze(2).to(x_enc.device)

            #print(x_enc_i.shape)
            #self.new_encoder = self.new_encoder.to(x_enc.device)
            
            _, hidden = self.encoder(x_enc_i)
            _, hidden = self.new_encoder[i](x_enc_i)
            ...
            if self.totrain: # teacher force training
                for t in range(self.pred_len):
                    x = x.unsqueeze(1)
                    #print("x1: ",x.shape)
                    output, hidden = self.new_decoder[i](x, hidden)
                    output = self.outputlinear(output.squeeze(1))
                    outputs[:,t] = output
                    x = x_dec_i[:,t,-self.dec_out:]
                ...
                for t in range(self.pred_len):
                    ...
                    outputs[:,t] = output
                    x = output
            #print("temp: ",outputs.shape)
            if i==0:
                final_outputs = outputs
            else:
                final_outputs = torch.cat([final_outputs,outputs],dim=2)
        #print("final: ",self.finaloutputlinear(final_outputs).shape)

        return self.finaloutputlinear(final_outputs)
