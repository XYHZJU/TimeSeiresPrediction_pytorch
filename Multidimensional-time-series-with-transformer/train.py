...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, max_len=5000):
        ...
        div_terme = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))
        div_termo = torch.exp(torch.arange(1, d_model, 2).float() * (-math.log(10000.0) / d_model))
        pe[:, 0::2] = torch.sin(position * div_terme)
        pe[:, 1::2] = torch.cos(position * div_termo)
...
class TransAm(nn.Module):
    def __init__(self,feature_size=23,num_layers=3,dropout=0.1):
...
def create_inout_sequences(input_data, tw):
    ...
    for i in range(L-tw):
        train_seq = np.append(input_data[i:i+tw,:][:-output_window,:] , np.zeros((output_window,23)),axis=0)
        train_label = input_data[i:i+tw,:]
        # print(train_seq.shape,train_label.shape)
        #train_label = input_data[i+output_window:i+tw+output_window]
        inout_seq.append((train_seq ,train_label))
    print("inout_seq:",len(inout_seq))
...
def get_data():
    ...
    data = np.loadtxt('data/dataset0722.txt', delimiter=',')
...
def train(train_data):
    ...
    for batch, i in enumerate(range(0, len(train_data) - 2 - input_window, batch_size)):
        ...
        if calculate_loss_over_all_values:
            ...
            loss = criterion(output[-output_window:,-3:], targets[-output_window:,-3:])
...
def plot_and_loss(eval_model, data_source,epoch,scaler):
    ...
    with torch.no_grad():
        for i in range(0, len(data_source) - 2 - input_window):
            ...
            if calculate_loss_over_all_values:                                
                ...
                total_loss += criterion(output[-output_window:,-3:], target[-output_window:,-3:]).item()
    ...
    pyplot.plot(test_result[:,-1],color="red")
    pyplot.plot(truth[:500,-1],color="blue")
...
def evaluate(eval_model, data_source):
    ...
    with torch.no_grad():
        for i in range(0, len(data_source) - 2 - input_window, eval_batch_size):
            data, targets = get_batch(data_source, i,eval_batch_size)
            output = eval_model(data) 
            print(output[-output_window:].size(),targets[-output_window:].size())
            if calculate_loss_over_all_values:
                total_loss += len(data[0])* criterion(output[:,-3:], targets[:,-3:]).cpu().item()
            else:                                
                total_loss += len(data[0])* criterion(output[-output_window:,-3:], targets[-output_window:,-3:]).cpu().item()            
